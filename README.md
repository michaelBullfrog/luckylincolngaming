...
- Clerk Chat outbound-SMS webhook matching calls within 30 minutes
- Dashboard SMS column displays Yes/No
...
CLERKCHAT_WEBHOOK_TOKEN=<optional-random-secret>
```
...
## Clerk Chat SMS webhook
Create an outbound-message webhook subscription in Clerk Chat and point it to:

```text
https://YOUR-RENDER-SERVICE.onrender.com/api/webhooks/clerkchat
```

If `CLERKCHAT_WEBHOOK_TOKEN` is set in Render, use:

```text
https://YOUR-RENDER-SERVICE.onrender.com/api/webhooks/clerkchat?token=YOUR_SECRET
```

The app normalizes the outbound recipient phone number and matches it to the most recent ended Webex call from the same number when the SMS was sent within 30 minutes after that call ended. A match sets `sms_sent=true`, and the dashboard displays **Yes**. Otherwise the call displays **No**.

Because Clerk Chat's public support documentation currently exposes webhook subscriptions but not a stable payload schema, v0.4 accepts several common field names and stores recent webhook metadata for verification. After the first real webhook arrives, inspect `GET /api/webhooks/clerkchat/recent` and tighten the mapping if needed.
