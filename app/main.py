...
from .models import Call, ClerkSmsEvent, SupervisorRerouteEvent, CallJourney, CallJourneyEvent
...
app = FastAPI(title="Lucky Lincoln Contact Center Analytics", version="1.0.0")
...
def _filtered_calls_stmt(
    *,
    ...
    stmt = select(Call)
    if outcome == "REROUTED_TO_SUPERVISOR":
        stmt = stmt.where(
            select(SupervisorRerouteEvent.id)
            .where(SupervisorRerouteEvent.task_id == Call.task_id)
            .exists()
        )
    elif outcome:
...
def _check_journey_token(request: Request, query_token: str | None):
    expected = os.getenv("CALL_JOURNEY_WEBHOOK_TOKEN") or os.getenv("SUPERVISOR_REROUTE_WEBHOOK_TOKEN")
...
@app.post("/api/webhooks/supervisor-reroute")
async def supervisor_reroute_webhook(
    request: Request,
    token: str | None = Query(default=None),
    db: Session = Depends(get_db),
):
    expected = os.getenv("SUPERVISOR_REROUTE_WEBHOOK_TOKEN")
    header_token = request.headers.get("X-Webhook-Token")
    if expected and token != expected and header_token != expected:
        raise HTTPException(status_code=401, detail="Invalid supervisor reroute webhook token")
    try:
        payload = await request.json()
    except Exception as exc:
        raise HTTPException(status_code=400, detail="Webhook body must be JSON") from exc
    ...
    task_id = payload.get("task_id") or payload.get("taskId") or payload.get("interaction_id") or payload.get("interactionId")
    if not task_id:
    ...
    ts = payload.get("rerouted_at") or payload.get("reroutedAt") or payload.get("timestamp")
    rerouted_ms = _now_ms()
    if ts is not None:
        try:
            if isinstance(ts, (int, float)):
                rerouted_ms = int(ts if ts > 10_000_000_000 else ts * 1000)
            else:
                dt = datetime.fromisoformat(str(ts).replace("Z", "+00:00"))
                if dt.tzinfo is None:
                    dt = dt.replace(tzinfo=timezone.utc)
                rerouted_ms = int(dt.timestamp() * 1000)
        except Exception:
            pass

    reason = payload.get("reason")
    supervisor_name = payload.get("supervisor_name") or payload.get("supervisorName")
    event_key = str(payload.get("event_id") or payload.get("eventId") or f"supervisor-reroute:{task_id}")
    raw_json = __import__("json").dumps(payload, sort_keys=True, separators=(",", ":"))

    event = db.scalar(select(SupervisorRerouteEvent).where(SupervisorRerouteEvent.task_id == task_id))
    created = event is None
    if event is None:
        event = SupervisorRerouteEvent(task_id=task_id, event_key=event_key, rerouted_ms=rerouted_ms, raw_json=raw_json)
        db.add(event)
    event.rerouted_ms = rerouted_ms
    event.reason = str(reason)[:255] if reason not in (None, "") else None
    event.supervisor_name = str(supervisor_name)[:255] if supervisor_name not in (None, "") else None
    event.raw_json = raw_json
    db.commit()

    matched = db.scalar(select(Call.id).where(Call.task_id == task_id)) is not None
    return {"ok": True, "stored": True, "created": created, "task_id": task_id, "matched": matched, "rerouted_at": _iso(rerouted_ms)}


@app.get("/api/webhooks/supervisor-reroute/recent")
def supervisor_reroute_recent(limit: int = Query(default=20, ge=1, le=100), db: Session = Depends(get_db)):
    rows = db.scalars(select(SupervisorRerouteEvent).order_by(SupervisorRerouteEvent.rerouted_ms.desc()).limit(limit)).all()
    return [{
        "task_id": r.task_id,
        "rerouted_at": _iso(r.rerouted_ms),
        "reason": r.reason,
        "supervisor_name": r.supervisor_name,
        "matched": db.scalar(select(Call.id).where(Call.task_id == r.task_id)) is not None,
    } for r in rows]
...
def dashboard_summary(
    ...
    rerouted_to_supervisor = db.scalar(
        select(func.count()).select_from(
            base.join(SupervisorRerouteEvent, SupervisorRerouteEvent.task_id == base.c.task_id)
        )
    ) or 0
    ...
    return {
        ...
        "sms_sent": sms_sent,
        "rerouted_to_supervisor": rerouted_to_supervisor,
...
def dashboard_calls(
    ...
    reroutes = {}
    if task_ids:
        reroutes = {r.task_id: r for r in db.scalars(
            select(SupervisorRerouteEvent).where(SupervisorRerouteEvent.task_id.in_(task_ids))
        ).all()}
    ...
    return [
        {
            ...
            "rerouted_to_supervisor": c.task_id in reroutes,
            "rerouted_at": _iso(reroutes[c.task_id].rerouted_ms) if c.task_id in reroutes else None,
            "reroute_reason": reroutes[c.task_id].reason if c.task_id in reroutes else None,
            "supervisor_name": reroutes[c.task_id].supervisor_name if c.task_id in reroutes else None,
...
def dashboard_filters(db: Session = Depends(get_db)):
    ...
    if "REROUTED_TO_SUPERVISOR" not in outcomes:
        outcomes.append("REROUTED_TO_SUPERVISOR")
