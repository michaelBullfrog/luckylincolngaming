...
def agent_states_during(
    ...
    *,
    ...
    """Return Contact Center telephony states for queue-eligible agents only."""
    ...
    for row in rows:
        ...
        if state in AVAILABLE_STATES:
            ...
            # but unavailable (idle/RONA, AUX/code, wrap-up, etc.).
...
def classify_task(
    ...
    *,
    ...
    states = agent_states_during(db, *queue_window, eligible_agent_ids=eligible_ids)

    # Exclusive call-level classification using Contact Center state only.
    # Available wins because it directly answers the question "could an observed
    # queue agent have taken this call?". Otherwise we distinguish busy from
    # logged-in/unavailable, then no logged-in eligible agents.
    if states["available"]:
...
def upsert_calls(db: Session, tasks: list[dict], *, availability_reliable: bool = True) -> int:
    ...
    for task in tasks:
        ...
        outcome, state_names = classify_task(
            db, task, availability_reliable=availability_reliable, queue_roster_cache=queue_roster_cache
        )
