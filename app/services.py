...
def agent_states_from_raw_sessions_at(
    ...
    *,
    ...
    """Resolve the signed-in telephony state for each eligible agent at one instant.

    Critical rule: choose exactly ONE agentSession per agent -- the most recent
    session whose startTime is on/before the queue-exit timestamp. Only after
    selecting that session do we decide whether it was still open at the sample.

    This prevents an older stale/open ASR from making an agent appear signed in
    after a newer session for that same agent has already ended.
    """
    empty = {"available": [], "on_call": [], "idle_code": [], "logged_in": []}
    if not eligible_agent_ids or queue_end_ms <= 0:
        return empty
    ...
    # First select ONE authoritative session per agent: the most recently-started
    # session at or before the call's queue-exit timestamp. Do this regardless of
    # whether the session later proves to be ended; a newer ended session means the
    # agent is logged out and an older stale session must not be considered.
    latest_session_by_agent: dict[str, dict] = {}
    for session in sessions or []:
        ...
        if session_start <= 0 or session_start > sample_ms:
            continue
        current = latest_session_by_agent.get(agent_id)
        current_start = int((current or {}).get("startTime") or 0)
        if current is None or session_start > current_start:
            latest_session_by_agent[agent_id] = session

    latest_state_by_agent: dict[str, tuple[int, str, str]] = {}

    for agent_id, session in latest_session_by_agent.items():
        session_start = int(session.get("startTime") or 0)
        session_end = int(session.get("endTime") or -1)
        is_active = session.get("isActive")

        # Definitive logout: the newest session had already ended before the call.
        if session_end > 0 and session_end <= sample_ms:
            continue

        # If Webex supplies no endTime, only trust the session as open when Webex
        # explicitly marks it active. An inactive open-ended historical ASR is not
        # enough evidence that the agent was signed in at this call timestamp.
        if session_end <= 0 and is_active is not True:
            continue
        ...
        for channel in session.get("channelInfo") or []:
            ...
            for node in ((channel.get("activities") or {}).get("nodes") or []):
                activity_start = int(node.get("startTime") or 0)
                activity_end = int(node.get("endTime") or -1)
                if activity_start <= 0 or activity_start > sample_ms:
                    continue

                # Respect the activity's own end when present. Open activities may
                # only survive because the authoritative parent session above is
                # still active at this timestamp.
                if activity_end > 0 and activity_end <= sample_ms:
                    continue

                state = (node.get("state") or "unknown").lower()
                candidate = (activity_start, state, agent_name)
                if best is None or candidate[0] > best[0]:
                    best = candidate

        # If there is no telephony activity covering the timestamp, do not invent
        # a state from an older record. The session may be logged in, but it cannot
        # be reported as Available without an exact activity proving that state.
        if best is not None:
            latest_state_by_agent[agent_id] = best
    ...
    for _agent_id, (_start, state, name) in latest_state_by_agent.items():
        buckets["logged_in"].add(name)
        if state in AVAILABLE_STATES:
            buckets["available"].add(name)
        elif state in ON_CALL_STATES:
