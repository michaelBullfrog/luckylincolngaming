...
from .webex import WebexError, activity_truncation_counts, fetch_agent_sessions, fetch_tasks, fetch_tasks_by_ended_time

Base.metadata.create_all(bind=engine)

app = FastAPI(title="Lucky Lincoln Contact Center Analytics", version="1.0.13")
...
async def _sync_window(db: Session, from_ms: int, to_ms: int) -> dict:
    ...
    tasks = await fetch_tasks_by_ended_time(from_ms, to_ms)
...
@app.get("/api/debug/latest-calls")
def debug_latest_calls(
    limit: int = Query(default=10, ge=1, le=100),
    db: Session = Depends(get_db),
):
    rows = db.scalars(
        select(Call).order_by(Call.created_ms.desc()).limit(limit)
    ).all()
    return {
        "count": len(rows),
        "latest_created_ms": rows[0].created_ms if rows else None,
        "latest_created_at": _iso(rows[0].created_ms) if rows else None,
        "calls": [
            {
                "task_id": c.task_id,
