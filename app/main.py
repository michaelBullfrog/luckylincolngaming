...
app = FastAPI(title="Lucky Lincoln Contact Center Analytics", version="0.7.1")
...
def reclassify_history(
    ...
    """Reclassify stored unserved calls using the tighter queue-window agent-state logic."""
    from_ms = _parse_date_ms(from_date)
    to_ms = _parse_date_ms(to_date, end_of_day=True)
    return {
        "ok": True,
