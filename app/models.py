...
class CallJourney(Base):
    __tablename__ = "call_journeys"

    id: Mapped[int] = mapped_column(primary_key=True)
    journey_id: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    caller_number: Mapped[str | None] = mapped_column(String(64), index=True)
    root_interaction_id: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    overflow_ms: Mapped[int] = mapped_column(BigInteger, index=True)
    returned_interaction_id: Mapped[str | None] = mapped_column(String(64), unique=True, nullable=True, index=True)
    returned_ms: Mapped[int | None] = mapped_column(BigInteger, nullable=True, index=True)
    status: Mapped[str] = mapped_column(String(64), default="OVERFLOWED", index=True)
    raw_json: Mapped[str | None] = mapped_column(Text)


class CallJourneyEvent(Base):
    __tablename__ = "call_journey_events"

    id: Mapped[int] = mapped_column(primary_key=True)
    event_key: Mapped[str] = mapped_column(String(128), unique=True, index=True)
    journey_id: Mapped[str] = mapped_column(String(64), index=True)
    interaction_id: Mapped[str | None] = mapped_column(String(64), index=True, nullable=True)
    event_type: Mapped[str] = mapped_column(String(64), index=True)
    event_ms: Mapped[int] = mapped_column(BigInteger, index=True)
