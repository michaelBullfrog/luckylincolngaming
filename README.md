...
## v0.7.1 classification

Calls with no `lastQueue` and no measurable queue wait are classified as `NOT_QUEUED_IVR_EXIT`. They remain visible in Total Calls and Call History but are excluded from the Unserved KPI. Use **Reclassify Agent States** after deployment to update existing stored calls in the selected date range.

## v0.8.1 Contact Center-only queue-state classification

This version does **not** use a separate Webex Calling/presence/configuration lookup. It relies only on the Contact Center data already imported into PostgreSQL:

- `taskDetails` identifies the queue and answered agent.
- `agentSession` / telephony activities provide `available`, `connected`, `ringing`, `inbound-reserved`, `idle`, `wrapup`, and other states.
- Queue eligibility is inferred conservatively from agents who have actually answered that same queue in stored Contact Center history.
- Unserved calls are evaluated only during the caller's estimated queue-wait window.
- If the stored history has never shown an agent answering a queue, the call is left as `UNSERVED_AVAILABILITY_UNKNOWN` rather than guessing.

After deploying, use **Reclassify Queue States** for the desired date range. No new Render environment variable is required.
